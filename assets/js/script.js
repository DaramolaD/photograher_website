...
const setLetterEffect = function () {
    ...
    for (let i = 0; i < letterBoxes.length; i++) {
        ...
        //get all charater from the current letter box
        const letters = letterBoxes[i].textContent.trim();

        //remove all charater from yhe current letter box
        ...
        for (let j = 0; j < letters.length; j++) {
            ...
            // add space class on span, when the current letter contain space
        ...
        // get total delay to active letter box
        if (i === activeLetterBoxIndex) {
...
const scrollReveal = function () {
    for (let i = 0; i < revealElements.length; i++) {
        const elementIsScreen = revealElements[i].getBoundingClientRect().top < window .innerHeight / 1.15;
